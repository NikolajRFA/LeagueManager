...
[Route("series")]
[ApiController]
public class SeriesController(SeriesDataService dataService, LinkGenerator linkGenerator, IMapper mapper)
    : GenericControllerBase(linkGenerator, mapper)
{
    ...
    public IActionResult GetSeries(int page = 0, int pageSize = 10)
    {
        ...
        foreach (var series in seriesList) dtos.Add(MapSeries(series));
    ...
    {
        dataService.PlaySeries(seriesModel.BlueSideTeamId, seriesModel.RedSideTeamId, seriesModel.BestOf,
            seriesModel.Date);
        return Ok();
    }
}